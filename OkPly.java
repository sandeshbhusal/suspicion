...
public class OkPly extends Bot {
    Random r = new Random();
                        ...
                        for (String card : allCardsArray) {
                            ...
                            for (String cardAction : card.split(":")) // just go ahead and do them in this order
                            {
                                if (cardAction.startsWith("move")) {
                                    ...
                                    actions += ":viewDeck";
                                    float averageGuessLength = calculateAverageGuessLength(cloned3);
                                    // Update kbGained using the average guess length
                                    kbGained += averageGuessLength;
    ...
    private float calculateAverageGuessLength(GameState gameState) {
        int totalGuessLength = 0;
        int playerCount = 0;
    
        for (String player : gameState.players.keySet()) {
            if (!player.equals(gameState.me)) {
                int guessLength = gameState.players.get(player).possibleGuestNames.size();
                totalGuessLength += guessLength;
                playerCount++;
            }
        }
    
        return (playerCount > 0) ? (float) totalGuessLength / playerCount : 0;
    }

    private float getGain(GameState gameState, String guest, String player, String board) {
        // possible character if the answer is true
        ...
        int remainingGuessTrue = remainingGuess(actualKB, possibleGuest_true);
        int remainingGuessFalse = actualKB.size() - remainingGuessTrue;
        
        float trueFrac = (float) remainingGuessTrue / actualKB.size();
        float falseFrac = (float) remainingGuessFalse / actualKB.size();

        int totalGuessLength = 0;
        int playerCount = 0;
    
        for (String otherPlayer : gameState.players.keySet()) {
            if (!otherPlayer.equals(player) && !otherPlayer.equals(gameState.me)) {
                int guessLength = gameState.players.get(otherPlayer).possibleGuestNames.size();
                totalGuessLength += guessLength;
                playerCount++;
            }
        }
    
        //float bestavg = Math.min((totalGuessLength + trueFrac)/playerCount, (totalGuessLength + falseFrac)/playerCount);
        float bestavg = Math.max((totalGuessLength + trueFrac)/playerCount, (totalGuessLength + falseFrac)/playerCount);

        float gain = 1 - ((trueFrac * trueFrac) + (falseFrac * falseFrac));
    ...
    public OkPly(String playerName, String guestName, int numStartingGems, String gemLocations, String[] playerNames, String[] guestNames) {
